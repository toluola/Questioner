...
describe('post meetups endpoint', () => {
	it('should post meetups', (done) => {
		...
		.end((err, res) => {
			...
			expect(res.body.createdMeetup.meetup_info).to.have.property("id");
			expect(res.body.createdMeetup.meetup_info).to.have.property("createdOn");
			expect(res.body.createdMeetup.meetup_info).to.have.property("location");
			expect(res.body.createdMeetup.meetup_info).to.have.property("images");
			expect(res.body.createdMeetup.meetup_info).to.have.property("topic");
			expect(res.body.createdMeetup.meetup_info).to.have.property("happeningOn");
			expect(res.body.createdMeetup.meetup_info).to.have.property("tags");
			expect(res.body)
				.to.have.property("message")
...
describe('meetups response endpoint', () => {
	it('should post response to meetups', (done) => {
		...
		.end((err, res) => {
			expect(res).to.have.status(201);
			expect(res.body.data.response_details).to.have.property("meetup");
			expect(res.body.data.response_details).to.have.property("topic");
			expect(res.body.data.response_details).to.have.property("topic");
			expect(res.body)
				.to.have.property("message")
...
describe('Get all upcoming meetups endpoint', () => {
	it('should fetch all upcoming meetups', (done) => {
		...
		.end((err, res) => {
			...
			expect(res.body.upcoming_meetups[0]).to.have.property("location");
			expect(res.body.upcoming_meetups[0]).to.have.property("status");
			expect(res.body.upcoming_meetups[0]).to.have.property("topic");
			expect(res.body.upcoming_meetups[0]).to.have.property("happeningOn");
...
describe('post questions endpoint', () => {
	it('should post questions', (done) => {
		...
		.end((err, res) => {
			expect(res).to.have.status(201);
			expect(res.body.createdQuestion.question_info).to.have.property("createdBy");
			expect(res.body.createdQuestion.question_info).to.have.property("meetup");
			expect(res.body.createdQuestion.question_info).to.have.property("title");
			expect(res.body.createdQuestion.question_info).to.have.property("body");
			expect(res.body)
				.to.have.property("message")
